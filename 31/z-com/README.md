...
회원가입 모달에서 서버 컴포넌트에서 서버 액션 사용

훅들을 없애야함

form 에 required 속성 있는 태그 필드 값이 비어있으면 버튼이 비활성화됨

form 데이터를 그대로 사용하므로 input 에 name 속성 추가해야함

클라이언트 컴포넌트 안에서 사용할 때는 함수 내에 "use server" 를 사용

서버 액션을 사용하면 서버 코드를 작성할 수 있음
- 브라우저에는 노출되지 않고 서버에서 실행됨
- 브라우저에 노출되지 않으므로 키 등을 활용할 수 있음

credentials: 'include' 을 fetch 에 포함하면 쿠키를 함께 사용
